...
export const summaryApi = {
  summarizeContent: (request: SummarizationRequest): Promise<SummarizationResponse> =>
    api.post('/summarization/summarize', request).then(res => res.data),
    
  summarizeArticle: (articleId: string, params?: { max_length?: number }): Promise<SummarizationResponse> =>
    api.post(`/summarization/articles/${articleId}/summarize`, params).then(res => res.data),
}
...
export const searchApi = {
  ...
  semanticSearch: (params: ArticleSearchParams & { query: string }): Promise<SearchResponse> =>
