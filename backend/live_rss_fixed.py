"""
AI Tech News Assistant Backend - Live RSS Feeds (Fixed Version)
==============================================================
...
# Simplified RSS Feed Sources (more reliable)
RSS_FEEDS = {
    "Hacker News": {
...
def safe_parse_date(date_str: str) -> datetime:
    ...
    try:
        ...
        try:
            ...
            # Fallback to feedparser's built-in parsing
...
def fetch_single_feed(feed_name: str, feed_info: dict) -> List[dict]:
    ...
    try:
        ...
        for entry in feed.entries[:8]:  # Limit to 8 articles per feed
            try:
                # Parse published date with safe fallback
                pub_date = datetime.now()
                if hasattr(entry, 'published'):
                    pub_date = safe_parse_date(entry.published)
                elif hasattr(entry, 'updated'):
                    pub_date = safe_parse_date(entry.updated)
                
                # Make sure pub_date is timezone-naive
                if hasattr(pub_date, 'tzinfo') and pub_date.tzinfo is not None:
                    pub_date = pub_date.replace(tzinfo=None)
                
                # Skip very old articles (older than 7 days for testing)
...
def update_articles():
    ...
    with update_lock:
        ...
        articles_storage = unique_articles[:50]  # Keep latest 50 articles
